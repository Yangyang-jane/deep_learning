...
class ComputeLossOTA:
    ...
    def find_3_positive(self, p, targets):
        ...
        ai = torch.arange(na, device=targets.device).float().view(na, 1).repeat(1, nt)  # same as .repeat_interleave(nt) num_anchor*num_target
...
class ComputeLossAuxOTA:
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        for i in range(self.nl):  # layer index, layer predictions
            ...
            if n_aux:#不同点，按照0.25的比例融合损失 aux head loss 和lead head loss 按照0.25:1的比例进行融合
    ...
    def build_targets2(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            top_k, _ = torch.topk(pair_wise_iou, min(20, pair_wise_iou.shape[1]), dim=1)#不同点，这里取20个
            dynamic_ks = torch.clamp(top_k.sum(1).int(), min=1)
