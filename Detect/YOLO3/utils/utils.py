...
def bbox_iou(box1, box2, x1y1x2y2=True):
    ...
    # get the corrdinates of the intersection rectangle 左上求最大，右下求最小就是交集
    inter_rect_x1 = torch.max(b1_x1, b2_x1) # b2中大于b1的不变，小于b1的都赋值为b1
    inter_rect_y1 = torch.max(b1_y1, b2_y1)
    inter_rect_x2 = torch.min(b1_x2, b2_x2)  # b2中小于b1的不变，大于b1的都赋值为b1
...
def non_max_suppression(prediction, conf_thres=0.5, nms_thres=0.4):
    ...
    prediction[..., :4] = xywh2xyxy(prediction[..., :4]) #前四个是x,y,w,h转化成xyxy左上右下
    output = [None for _ in range(len(prediction))]
    for image_i, image_pred in enumerate(prediction):
        # Filter out confidence scores below threshold
        image_pred = image_pred[image_pred[:, 4] >= conf_thres] # 置信度大于阈值
        ...
        # Sort by it
        image_pred = image_pred[(-score).argsort()] # argsort()函数返回的数组是：按照数组值从小到大的index值（这里score加负号表示从大到小排序）
        class_confs, class_preds = image_pred[:, 5:].max(1, keepdim=True)
        detections = torch.cat((image_pred[:, :5], class_confs.float(), class_preds.float()), 1) # x,y,x,y,conf_object,p_class_max,class
        # Perform non-maximum suppression 极大值抑制
        keep_boxes = []
        while detections.size(0):
            large_overlap = bbox_iou(detections[0, :4].unsqueeze(0), detections[:, :4]) > nms_thres # iou阈值
            label_match = detections[0, -1] == detections[:, -1]
            # Indices of boxes with lower confidence scores, large IOUs and matching labels
            invalid = large_overlap & label_match   # 位运算，两者相同为结果，两者相反为false
